...
5.  交替结构
    ...
    图像压缩算法在之前的图中已提及，最流行的包括一些形式的预测反馈，比如下图所示。在这个例子中，使用了一个散量化来转换采样，可能被一一量化随着一个激光扫描从采样数组头行到底行且每行为从左到右。不是通过 $ y[k] = y[k_ {1}, k_ {2}] $的直接量化采样，我们量化预测预留
    
    ![img](../img/modified_compression_structure_with_predictive_feedback.png)
    
    $ e[k] = y[k] - y_ {p}[k] $

    预测器 $ y_ {p}[k] $，是重建采样函数 $ \\hat{y}[n] $。要么 $ n_ {1} < k_ {1}, 或 n_ {1} = k_ {1} 且 n_ {2} < k_ {2} $。这些采样以被解码器重建，这样相同的预测器，$ y_ {p}[k] $，被压缩器和解压器计算使用

    预测反馈背后的思想是预测预留，$ e[k] $，应该一般接近为0。在量化之后，量化索引的统计分布应该偏向量化间隔包含0的索引。结果统计冗余被适合的编解码算法应用。最简单的预测是之前重建的采样，$ y_ {p}[k_ {1}, k_ {2}] = \\hat{y}[k_ {1}, k_ {2} - 1] $；历史原因，这被称为DPCM（差分脉冲编码调制）。另一个例子，我们将使用之前采样的平均值到预测的左边和上边；例如，$ y_ {p}[k_ {1}, k_ {2}] = \\frac{1}{2}(\\hat{y}[k_ {1}, k_ {2} - 1], \\hat{y}[k_ {1} - 1, k_ {2}]) $

    预测反馈价值不大除非转换失败需要移除多数空间冗余 - 回忆转换的一个关键目标是最小化采样间的统计交互。因这个原因，该形式的预测反馈如上图通常用来替代转换，如下图。在这个例子中，多个图片一个个被压缩，预测被之前重建图像形成。下图的反馈结构基于流行视频压缩方案比如那些被CCITT H.261和H.263嵌入的视频电话标准和被MPEG（Motion Picture Experts Group）工作组发展的ISO/IEC标准。其他预测反馈概念的变种可在无损压缩图像算法和一些被JPEG图像压缩标准支持的模式中发现
    
    ![img](../img/predictive_feedback_structure_for_compressing_multiple_images.png)
    
    反馈压缩结构的一般特征是他们依赖压缩器的能力来精确复制一些或所有采样，其被解压器重建。事实上，上述两图揭示了压缩器必须包含解压器某些元素的精确拷贝
    
    一个关键的需求驱动JPEG2000标准化进程是扩张性。一个可扩展的比特流是可被部分丢弃来获得原图像的有效表示或不同比特率的低清晰度版本。高可扩展比特流可被许多不同方式解压为不同结果，取决于什么信息被丢弃。对压缩器困难的是重复解压器可能获得的所有可扩展压缩比特流的状态
