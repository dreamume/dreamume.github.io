...
1.  上下文
    ...
    如果你使用 GCC 的话，你应该会想用 <span class="underline"><span class="underline">PRETTY_FUNCTION_</span></span> 来替代 <span class="underline"><span class="underline">func</span></span> [[1]​](https://gcc.gnu.org/onlinedocs/gcc/Function-Names.html)。而如果你使用 MSVC，\__FUNCSIG_\_\_ 可能会是个更好的选择 [[2]​](https://docs.microsoft.com/en-us/cpp/preprocessor/predefined-macros?view=msvc-170) 
...
3.  记录上下文
    ...
    alloc\_mem_ 接受大小、上下文和对齐值（默认为系统的默认对齐值 \__STDCPP__DEFAULT__NEW__ALIGNMENT_\_\_），进行内存分配，并把上下文记录下来。显然，下面的问题就是：
    ...
    从 alloc\_list\__t_ 分出一个 alloc_list\__base_ 子类的目的是方便我们统一对链表头尾的操作，不需要特殊处理。alloc_list_ 的 next 成员指向链表头，prev 成员指向链表尾；把 alloc_list_ 也算进去，整个链表构成一个环形
