...
我们的系统呈现广谱一致性和性能折中的一个点。一致性模型，像其他系统一样，已经过多年演练来适配了网站的扩展。它混合了不牺牲我们的高性能需求地实际。系统管理的大容量数据意味着任何小的增加网络或存储需求的变动对它们有不可忽视的成本。大部分建议提供更严格语义而不离开设计阶段因为它们变得不可接受的昂贵。不像许多系统对现存的用例有一个长尾，memcache 和 Facebook 一起发展。这允许应用程序和系统工程师工作在一起来找到一个模型对应用程序工程师更容易理解，执行和足够简单地在扩展中能可靠地工作。我们提供最好的最终一致性但在性能和有效性上使用一个抽象。这样系统的工作在实际中对我们非常好且我们认为我们已找到一个可接受的折中
...
我们早起的决定需要存储簇通过后端程序无效化数据在多领域架构中有一个重要的顺序。特别地，它避免一个竞技条件一个无效化在数据还没有从主领域复制之前到达。考虑在主领域的一个 Web 服务器已经完成一个数据库的修改且寻找陈旧的数据来无效化。在主领域发送无效化是安全的。然而，在复制领域含有 Web 服务器含有无效化数据可能会过早因为改变还没有广播到复制数据库。从复制域的子查询将竞争复制流因此增加设置陈旧数据到 memcache 的可能性。历史上，我们在扩展到多领域后实现了 mcsqueal
...
现在考虑一个用户从一个非主领域更新他的数据当复制延迟过大时。用户的下一个请求可导致混乱如果他最新的改变丢失了。一个缓存从一个复制数据库重新填满将在复制流追赶上了之后被允许。否则，子请求会拿到在获取并缓存之前的复制节点脏数据

我们使用一个远端标签机制来最小化读脏数据的可能性。标准的呈现显示本地复制数据库的数据可能是脏的且查询应该转向主领域。当一个 Web 服务器希望更新数据影响一个键 k，服务器 (1) 设置一个远端标签 $ r\_ {k} $ 在领域中，(2) 执行写到主嵌入 k 和 $ r\_ {k} $ 在 SQL 语句中被无效化，且 (3) 在本地簇中删除 k。一旦有一个 k 的子请求，一个 Web 服务器将不会在缓存数据中找到，检查 $ r\_ {k} $ 是否存在，且定向查询到主或本地域取决于 $ r\_ {k} $ 的呈现。这个情况中，我们直接折中额外的延迟当一个缓存未命中时，减少读脏数据的可能性

我们通过使用一个领域池实现远端标签。注意这个机制可能在并行修改相同的键时显示脏数据因为一个操作可能删除一个远端标签其应该仍然呈现在另一个正在进行的操作时。值得提及的是我们 memcache 的使用对远端标签跟缓存结果很微妙地分开了。作为缓存，删除或驱逐键总是一个安全的行为，它可导致更多的数据库加载，但不削弱一致性。相反，一个远端标签的呈现帮助区分是否一个非主数据库持有脏数据。实际上，我们发现远端标签的驱逐和并行修改很少发生
...
领域间通信非常昂贵因为数据不得不穿越很大的地理距离（例如，跨美国大陆）。通过对删除流共享相同的通信频道这样数据库复制在低带宽连接中网络会更高效

上述管理删除的系统也采用复制数据库节点来广播删除到复制领域的 memcached 服务器。数据库和 mcrouters 缓存当下流组件变得不响应时删除。在任意组件中一个故障或延迟增加脏数据读取的可能性。当下流组件再次有效时缓存的删除会重放。当检测到问题时，替代者包括发生一个簇脱机或过度无效化前端簇数据。这些处理对我们的工作负载更加混乱多过利益
...
所有到所有的通信范型意味着单个服务器可变成一个簇的瓶颈。这节描述 memcached 中性能优化和内存效率以更好的在簇中扩展。改进单个服务器缓存性能是一个活跃的研究领域
...
我们开始用一个单线程 memcached，其使用一个固定大小的哈希表。第一个主要的优化为 (1) 允许自动扩展哈希表来避免查找时间复杂度变成 O(n)，(2) 使服务器多线程化，使用一个全局锁保护数据结构，且 (3) 给每个线程一个 UDP 端口来减少竞争当发送回复且之后扩散过度打断进程。头两个优化共享给了开源社区。这节后续内容探讨不在开源版本的进一步的优化

我们的实验宿主为一个 Intel Xeon CPU(X5650) 运行在 2.67GHz（12 核和12 个超线程），一个 Intel 82574L GB 以太网控制器和 12GB 内存。产品服务器有额外的内存。进一步的细节之前已发布。性能测试构建包含 15 个客户产生 memcache 流量到单个 24 个线程的 memcached 服务器。客户端和服务器同架且通过 GB 以太网连接。这些测试测量在两分钟满加载 memcached 回复的延迟

1.  get 性能

    ![img](../img/multiget_hit_and_miss_performance_comparison_by_memcached_version.png)

    我们首先调研加锁情况下替代我们原始多线程单锁实现的影响。我们通过在处理每 10 个键的 memcached 请求之前预先填充 32 字节的值到缓存来度量命中率。上图显示不同版本的 memcached 可维护的每办毫秒平均响应时间的最大请求率。第一个柱块是加锁前我们的 memcached，第二个柱块是我们当前的 memcached，且最后是开源版本 1.4.10 对我们的加锁策略独立实现的强制版本

    使用加锁的版本每秒从 600k 到 1.8M 条目命中率是峰值的三倍。对未命中的性能也从每秒 2.7M 提升到 4.5M。命中更昂贵因为返回值需要构建和转换，而未命中需要单个静态响应来对整个多 get 显示所有键缺失

    ![img](../img/get_hit_performance_comparison_for_single_gets_and_10_keys_multigets_over_tcp_and_udp.png)

    我们也调研使用 UDP 替代 TCP 的影响。上图显示我们可维护的对单个 get 和 10 个键的多个 get 小于 1 毫秒的平均延迟的峰值请求率。我们发现我们的 UDP 实现比我们的 TCP 实现对单 get 有 13% 的提升，10 个键的多 get 有 8% 的提升

    因为多 get 比单 get 在每个请求中压缩更多的数据，它们对相同的工作使用更少的包
...
Memcached 采用一个 slab 分配器来管理内存。分配器组织内存到 slab 类，每个包含预分配、统一大小内存簇。Memcached 存储条目在最小可能能放入条目 metadata、键和值 的 slab 类。slab 类在 64 字节处开始且以因子 1.07 指数级增长到 1MB，4 字节对齐。每个 slab 类维护一个有效簇的 free-list 且在 1MB slab 中需要更多的内存如果它的 free-list 为空。一旦一个 memcached 服务器不能再分配内存，通过在这个 slab 类中驱除最少最近使用（LRU）的条目。当工作负载改变，原始的内存分配到每个 slab 类可能不再足够在池命中率上

我们实现一个视频分配器周期重平衡 slab 分配来匹配当前的工作负载。它定义 slab 类作为需要更多的内存如果它们驱逐条目且如果下一个被驱逐的条目被使用至少高 20% 相比其他 slab 类平均最近最少使用的条目。如果这样一个类被发现，则 slab 持有最近最少使用条目被释放且转换到需要的类。注意开源社区独立实现一个相似的分配器平衡跨 slab 类的驱逐率当我们的算法聚焦于平衡类间老条目的使用年龄。平衡年龄提供一个更好的估计对单个全局最少最近使用（LRU）驱逐策略对整个服务器而不是调整严重影响访问范型的驱逐率
...
当 memcached 支持过期时间，条目可在过期之后仍然在内存中存活。当处理条目一个 get 请求或当条目到达 LRU 尾部时Memcached 通过检查过期时间懒驱逐这样的条目。虽然对普通情况高效，这个方案允许看到一个单爆发活动的短活键浪费内存直到它们到达 LRU 尾部

我们因此引入一个混合方案依赖对大多数键的懒驱逐及主动驱逐过期的暂活键。我们把短活条目放入一个链表的环缓存中（以秒做索引直到过期）- 称为暂活条目缓存 - 基于条目的过期时间。每秒，缓存区头部 bucket 的所有条目被驱逐且头部往前走一个。当我们添加一个短过期时间到一个重使用键集合，这些键只有短暂的有用生命周期；被这个键族使用的 memcache 池比例从 6% 减少到 0.3% 且不影响命中率
...
由于升级、bug 修复、临时诊断、或性能测试导致频繁的软件变更。一个 memcached 服务器可在数小时内达到它峰值命中率的 90%。结果，它需要我们超过 12 个小时来升级一个 memcached 服务器集合因为结果的数据库加载需要小心管理。我们修改 memcached 存储它的缓存值和主数据结构在系统 V 共享内存区域这样数据可仍然在软件升级时存活且最小化干扰
...
我们现在使用产品中服务器使用的数据来特征化 memcache 工作负载
...
我们记录一小比例用户请求的所有 memcache 操作并讨论输出、响应大小和我们工作负载的延迟特征

1.  输出

    ![img](../img/cumulative_distribution_of_the_number_of_distinct_memcached_servers_accessed.png)
    
    上图显示当回应一个请求时一个 Web 服务器可能需要联系的不同 memcached 服务器的分布。56% 的所有网页请求联系少于 20 个 memcached 服务器。用户请求趋于询问小数量的缓存数据。然而，这个分布有一个长尾。图片也描述了对我们更流行页面之一的分布更好地展示所有到所有的通信模型。这种类型的多数请求将访问超过 100 个不同的服务器；访问数百个 memcached 服务器并不罕见

2.  响应大小

    ![img](../img/cumulative_distribution_of_value_sizes_fetched.png)
    
    上图显示 memcache 请求的响应大小。中位数（135 字节）和平均数（954 字节）的不同意味着缓存条目大小的巨大差异。另外出现了 3 个不同的峰值在大约 200 字节和 600 字节之间。更大的条目趋于存储数据列表而更小的条目趋于存储单片内容

3.  延迟

    我们度量从 memcache 请求数据的忘返延迟，包括请求发起到收到回复的网络传输时间和反序列化和解压的成本。7 天的请求延迟中位数是 333 毫秒，75% 和 95% 的是 475 微秒和 1.135 毫秒。从一个空闲 Web 服务器端到端的延迟中位数是 178 微秒而 75% 和 95% 的是 129 微秒和 374 微秒。在 95% 延迟中变化主要在处理大响应和等待运行线程被调度
...
<thead>
<tr>
<th scope="col" class="org-left">池</th>
<th scope="col" class="org-right">未命中率</th>
<th scope="col" class="org-left">get/s</th>
<th scope="col" class="org-left">set/s</th>
<th scope="col" class="org-left">delete/s</th>
<th scope="col" class="org-left">packets/s</th>
<th scope="col" class="org-right">outbound bandwidth (MS/s)</th>
</tr>
</thead>

<tbody>
<tr>
<td class="org-left">野</td>
<td class="org-right">1.76%</td>
<td class="org-left">262k</td>
<td class="org-left">8.26k</td>
<td class="org-left">21.2k</td>
<td class="org-left">236k</td>
...
<td class="org-right">7.85%</td>
<td class="org-left">96.5k</td>
<td class="org-left">11.9k</td>
<td class="org-left">6.28k</td>
<td class="org-left">83.0k</td>
...
<td class="org-left">710k</td>
<td class="org-left">1.75k</td>
<td class="org-left">3.22k</td>
<td class="org-left">44.5k</td>
<td class="org-right">30.1</td>
</tr>


<tr>
<td class="org-left">领域</td>
<td class="org-right">6.35%</td>
<td class="org-left">9.1k</td>
<td class="org-left">0.79k</td>
<td class="org-left">35.9k</td>
<td class="org-left">47.2k</td>
...
<tr>
<th scope="col" class="org-left">池</th>
<th scope="col" class="org-left">平均值</th>
<th scope="col" class="org-left">std dev</th>
<th scope="col" class="org-right">p5</th>
<th scope="col" class="org-right">p25</th>
<th scope="col" class="org-right">p50</th>
<th scope="col" class="org-right">p75</th>
<th scope="col" class="org-left">p95</th>
<th scope="col" class="org-left">p99</th>
</tr>
</thead>
...
<tr>
<td class="org-left">野</td>
<td class="org-left">1.11 K</td>
<td class="org-left">8.28 K</td>
<td class="org-right">77</td>
<td class="org-right">102</td>
<td class="org-right">169</td>
<td class="org-right">363</td>
<td class="org-left">3.65 K</td>
<td class="org-left">18.3 K</td>
</tr>


<tr>
<td class="org-left">app</td>
<td class="org-left">881</td>
<td class="org-left">7.70 K</td>
<td class="org-right">103</td>
<td class="org-right">247</td>
<td class="org-right">269</td>
<td class="org-right">337</td>
<td class="org-left">1.68 K</td>
<td class="org-left">10.4 K</td>
</tr>
...
<tr>
<td class="org-left">领域</td>
<td class="org-left">31.8 K</td>
<td class="org-left">75.4 K</td>
<td class="org-right">231</td>
<td class="org-right">824</td>
<td class="org-right">5.31 K</td>
<td class="org-right">24.0 K</td>
<td class="org-left">158 K</td>
<td class="org-left">381 K</td>
</tr>
</tbody>
</table>

我们现在讨论四个 memcache 池的关键度量。池是野的（缺省池），app（为一个特殊应用程序提供的池），一个对频繁访问数据的复制池和一个极少访问信息的领域池。在每个池中，我们收集每 4 分钟的平均统计和在上面第一个表中报道一个月收集周期的最高平均值。该数据估计这些池的峰值加载。该表显示不同池的 get、set、delete 率。第二个表显示每个池的响应大小的分布。不同的特征激活我们的预想去分割这些工作负载

我们在一个池中复制数据且利用批量处理高请求率。观察到复制池有最高的 get 率（大约 2.7 倍第二高的池）和对包最高字节率尽管有最小条目大小。该数据于我们的设计一致，我们平衡复制和批量来获取更好的性能。在 app 池中，一个更高的数据簇结果为一个自然地更高的未命中率。该池趋于有数小时内访问的内容且然后用更新的内容以流行度替换到老数据。领域中的数据趋于大型且不频繁访问的请求率和数据大小分布
...
我们认识到无效时间线是一个关键因素决定暴露脏数据的概率。为监控这个健康，我们采样百万分之一的删除和记录删除时间。我们按序查询常规间隔对采样键和如果一个条目仍然缓存尽管一个删除应该无效化它的错误日志在跨所有前端簇时 memcache 的内容

![img](../img/latency_of_the_delete_pipeline.png)

如上图，我们使用这个监控机制来报道跨 30 天范围内我们的无效延迟。我们分割这个数据为两个不同的部分：(1) 在主领域从一个 Web 服务器引起的删除和在主领域中目的地为一个 memcached 的服务器 (2) 从一个复制领域引起的删除和目的地为另一个复制领域的。作为数据展示，当删除的源和目的地在主领域时我们的成功率更高且在 1 秒内取得 4 个 9 的可靠性和一小时后5 个 9。然而当删除从源到位于主领域外部我们的可靠性掉到 1 秒内 3 个 9 和 10 分钟内 4 个 9。在我们的经验中，我们发现如果一个无效未命中在数秒之后，最常见的原因是第一个尝试失败且后续尝试将解决这个问题
...
一些其他大型网站意识到使用键值存储。DeCandia 等呈现了一个高有效键值存储用于 Amazon.com 上各种应用程序服务。当它们的系统为重写工作负载优化，我们的目标是一个被读支配的工作负载。相似地，LinkedIn 使用 Voldmort，一个激发于 Dynamo 的系统。其他主要的键值缓存解决方案的发展包括 Github、Digg 的 Redis 和 Blizzard，和 Twitter、Zynga 的 memcached。Lakshman 发展了 Cassandra，一个基于分布式键值存储的方案。我们更喜欢开发和扩展 memcached 由于它更简单的设计

我们在扩展 memcache 的工作构建在分布式数据结构现存的工作上。Gribble 呈现了一个对互联网扩展服务有用的键值存储系统的早期版本。Ousterhout 也呈现了对大型扩展内存键值存储系统的例子。跟这两个解决方案不一样，memcache 不保证持久化。我们依赖其他系统来处理持久化数据存储

Ports 提供一个库来管理查询的缓存结果到一个交易数据库。我们的需要需要一个更灵活的缓存策略。我们使用租赁和脏读平衡之前在缓存一致性和高性能系统的读操作的研究。Ghandeharizadeh 和 Yap 的工作也呈现了一个算法处理脏集问题基于时间戳而不是直接的版本号

而软件路由器更容易自定义和编程，它们比硬件组件性能稍差。Dobrescu 通过在一般服务器上利用多核、多内存管理器、多队列网络接口和批处理优势处理这些问题。应用这些技术到 mcrouter 实现仍然是未来的工作。Twitter 也独立开发一个 memcache 代理功能跟 mcrouter 相似

在 Coda、Satyanarayanan 显示了数据集如何通过断联操作分离来实现同步。Glendenning 均衡了 Paxos 和法定人数来构建 Scatter，一个分布式哈希表，其线性语义适应。Lloyd 检查在 COPS 里的随机一致性，一个广域存储系统

TAO 是另一个 Facebook 系统严重依赖缓存来服务大量低延迟查询。TAO 在两个基本的地方不同于 memcache。(1) TAO 实现一个图数据模型，其节点由一个固定长度的唯一号确定（64 比特整数）。(20) TAO 编码一个图模型到持久化数据的映射且负责持久化。许多组件，比如我们的客户库和 mcrouter，使用这两个系统
