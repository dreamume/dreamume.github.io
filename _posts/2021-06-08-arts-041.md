...
$ \\mathbb{P}(A \\land B) = \\mathbb{P}(A) \\dot \\mathbb{P}(B) $

**例子** 对一个骰子，投出偶数的事件A，和投出小于等于3的事件B是不独立的，因为 $ \\mathbb{P}(A) \\dot \\mathbb{P}(B) \\ne \\mathbb{P}(A \\land B) $。即 $ \\frac{1}{2} \\dot \\frac{1}{2} \\ne \\frac{1}{6} $。然而，如果你定义C为投出1或2的事件，则A和C是独立的，因为 $ \\mathbb{P}(A) = \\frac{1}{2}, \\mathbb{P}(C) = \\frac{1}{3}, \\mathbb{P}(A \\land C) = \\frac{1}{6} $
