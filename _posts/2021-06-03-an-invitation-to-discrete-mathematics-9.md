...
**更多关于无 $ k\_ {2,2} $及很多边的图形** 之前的定理告诉我们如果G是一个m个点的图形，且没有 $ K_ {2,2} $的子图形，则G有最多 $ \\frac{1}{2} (m^{3/2} + m) $条边。使用finite projective plane，我们显示该边界接近一般化上最好的可能
...
对m的无穷多个值，存在一个m个边的无 $ K_ {2,2} $的图形有至少 $ 0.35 m^{3/2} $条边
