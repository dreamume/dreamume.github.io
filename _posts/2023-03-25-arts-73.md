...
4.  索引提取
    ...
    我们的实现通过使用统计尾部 0 个数操作（通过 tzcnt 指令）和一个清除最低位操作 s = s & (s - 1) 其编译成一个 blsr 指令包含了一个位集到索引的转换。这个策略包含一个未预测的分支；除非有一个位集的规则范型，我们希望对每个双字有至少一个分支缺失。然而，我们使用一个技术从我们的位集中无条件地提取 8 个索引，然后忽略在下一次迭代的索引提取中过度提取的任意索引。如下
    
        // we decode the set bits from 's' to array 'b'
        uint64_t s = ...;
        uint32_t *b = ...;
...
5.  字符编码验证
    ...
    我们的经验，JSON 文档使用 UTF-8 unicode 格式编码。一些编程语言像 Java 使用 UTF-16 作为内存字符串。如果我们考虑流行的 Java JSON 库 Jackson，则序列化一个对象到 JSON 通常的方法是调用函数 ObjectMapper.writeValue，且结果是 UTF-8。JSON 指导说明显示许多实现不支持 UTF-8 以外的编码。解析器像 Mison 假设字符集为 ASCII。虽然它是有原因的，一个更安全的假设是使用 UTF-8。不是所有的字节序列都是有效的 UTF-8 且一个验证解析器需要确保字符集编码是正确的。我们假设数据是 UTF-8 的，且解析器生成的也是 UTF-8 字符串
    
    UTF-8 是 ASCII 的超集。ASCII 字符可用单个字符表示。JSON 之外的字符必须是 ASCII。由于在文档中可能有许多小字符串，所以不清楚是否向量化 unicode 验证在单个字符串层级是否受益。这样我们把输入字节作为一个整体验证
    
    我们首先测试是否一个 64 字节的块完全是 ASCII 字符。可通过验证所有字节的最高位是否为 0。这个优化可能会触发一些不可预测分支，但由于 JSON 文档经常是由完全的 ASCII 字符组成，这是必要的风险
    
    ![img](../img/unicode_code_points_with_four_bytes_sequences_in_utf8.png)
    
    如果有非 ASCII 字符，我们应用一个向量化 UTF-8 验证算法。我们需要检查字节序列由有效的 UTF-8 码点组成。它包含几个步骤，但每步都是高效的。我们使用 SIMD 指令：
    
    -   我们需要验证所有字节值不大于 0xF4：我们可通过对 0xF4 做一个 8 位减法获得这个检查。减法的结果是零当且仅当值不大于 0xF4
    -   当字节值 0xED 发现时，下一个字节必须不大于 0x9F；当 0xF4 发现时，下一个字节值不大于 0x8F。我们可用向量化字节比较和字节位移来检查这些条件
    -   字节值 0xC0 和 0xC1 是禁止的。当字节值为 0xE0，下一个字节值大于 0xA0。当字节值为 0xF0，下一个字节值至少为 0x90
    -   当一个字节值不在 ASCII 范围内时，它属于四种类型之一，取决于它的高位：
        -   如果高位是 8，9，a 或 b（十六进制）则字节是四字节码点的第二，第三字节
        -   如果高位是 c 或 d 则为两字节码点的第一个字节
        -   如果高位是 e 则为三字节码点的第一个字节
        -   如果高位为 f 则为四字节码点的第一个字节
    
    我们使用 vpshufb 指令快速映射字节到这几个类型之一，使用值 0，2，3 和 4。我们映射 ASCII 字符到值 1。如果发现值 4（对应高位为 f），它应该后接三个 0 值。给出这样一个整数向量，我们可检查它如下方式的有效码点序列。通过移动 1 位减 1，并相加到结果的方法，如下例：序列 4 0 0 0 2 0 1 1 3 0 0，我们首先得到 4 3 0 0 2 1 1 1 3 2 0，然后是 4 3 2 1 2 1 1 1 3 2 1。如果序列是有效的 UTF-8，所有最后的值应该大于 0，且不大于原始的向量
    
    所有这些检查只使用 SIMD 指令，没有分支。在处理的开始，我们初始化一个错误变量（作为一个 32 字节向量）为 0。我们用 OR 操作用我们的错误向量计算每个检查结果。如果检查有错误，则错误向量变成非零。我们只检查处理的结尾时变量为 0。如果一个诊断需要决定错误发生的地点，我们可在输入上做第二次处理
...
在最后的阶段，我们迭代第一阶段发现的索引。为处理嵌套的对象和数组，我们使用一个基于 goto 的状态机。我们的状态记录为一个堆栈显示是否我们在一个对象或数组中，我们添加我们的新状态到堆栈当我们嵌入到一个数组或对象时。当嵌入对象或数组结束时，我们使用堆栈中存储的状态且用 goto 命令重置到包含领域适合的状态。像 true，false，null 这样的值作为简单的字符串比较处理。我们使用修饰的函数解析数字和字符串。不需要很多额外的处理，我们即可支持无需把 JSON 文档对象作为内存磁带地字节流处理

1.  数值解析

    不用标准字符方式解析和验证数值比较困难。我们必须检查指导说明的所有规则。然而，我们发现通常当有至少有八个数字作为数值的小数部分是很有用的。给定八个字符解释为一个 64 位整数值，我们可用一个廉价地检查是否它由八个数字组成：
    
        (((val & 0xF0F0F0F0F0F0F0F0) | 
        (((val + 0x0606060606060606) & 0xF0F0F0F0F0F0F0F0) >> 4))
               == 0x3333333333333333)
    
    当这个检查成功时，我们调用一个快速向量化函数计算对应的整数值（见下图）。这个快速函数开始从所有字符值上减去字符 '0' 的码点值，使用 \_mm_sub__epi8_（使用 psubb 指令）。因为数字在 ASCII 中有连续的码点，这样使得数字字符映射到对应的值，比如 '0' 映射到 0，'1' 映射到 1 等等。我们然后调用 \_mm_maddubs__epi16_（比如，pmaddubsw 指令）来对其他数字乘以 10 且把结果添加到之前的数字，作为一个 16 位和。我们用 \_mm_madd__epi16_（例如，pmaddwd 指令）重复一个相似的处理，这次对每个其他值乘以 100 且添加它到之前的值作为一个 32 位和。这些和的最大值是 16 位整数 9999。我们应用 \_mm_packus__epi32_（例如 packusdw 指令）来把 4 个 32 位整数放入 4 个 16 位整数中。最后，我们调用 \_mm_madd__epi16_ 来对其他 16 位值乘以 1000 且添加它到之前的值， 产生一个 32 位和。我们只返回一个 32 位值对应 8 个数字，我们的函数转换两个系列的 8 数字为 32 位整数。在一个 Skylake 处理器上，指令有 1 个周期（psubb, packusdw）或 5 个周期（pmaddubsw, pmaddwd）的延迟。因为 pmaddwd 指令需要执行两次，我们可估计这个函数有至少 17 个周期的延迟，不包括存储和加载指令。然而，只有大约 7 个指令是需要的（包含加载/存储指令），当用一个本地处理时其可以减少一半。实际上，只有加载 8 个数字需要 8 个不同的加载指令
        ...
        uint32_t parse_eight_digits_unrolled(char *chars) {
          ...
          __m128i t4 = _mm_madd_epi16(53, mul_1_10000);
...
2.  字符串验证和正规化
    ...
    当统计一个引用字符，我们总是在一个向量寄存器读取 32 字节，然后查找引号和转义字符。如果在第一个引号之前发现一个转义字符，我们使用一个方便的代码路径来处理转义字符，否则我们只写 32 字节寄存器到我们的字符串缓存。一些 unicode 字符需要一到四个字符表示（例如，\uD834\uDD1E）；我们转换它们到有效的 UTF-8 字节
    
    我们的字符串缓存由一个 32 位整数表示字符串的长度，然后接 UTF-8 的字符串内容。JSON 指导说明允许字符串包含空字符，因此需要一个整数说明字符串长度。作为字符串验证的部分，我们需要检查没有小于 0x20 的码点值：我们使用向量化比较
...
我们通过一系列可重复产生变化数据的实验验证我们的结果。后续章节中我们报道在我们的两个阶段间分别解析的运行时间。及我们将展示跟我们最好的竞争产品相比我们只使用了一半的指令数。最后我们展示这些缩减的指令统计会转换为一个可比较的运行时优势
...
多数最新的 Intel 处理器基于 Skylake 微架构。在我们的测试中我们也包含一个计算机用更新的 Cannon Lake 微架构。我们在下表中总结了我们硬件平台的特征。我们实验性地用 avx-turbo 验证 CPU 特性
...
<th scope="col" class="org-left">Base Frequency</th>
<th scope="col" class="org-left">Max. Frequency</th>
<th scope="col" class="org-left">Microarchitecture</th>
...
<tbody>
<tr>
<td class="org-left">Intel i7-6700</td>
<td class="org-left">3.4 GHz</td>
<td class="org-left">3.7 GHz</td>
<td class="org-left">Skylake (x64, 2015)</td>
<td class="org-left">DDR4 (2133 MT/s)</td>
<td class="org-left">GCC 9.1</td>
</tr>


<tr>
<td class="org-left">Inter i3-8121U</td>
<td class="org-left">2.2 GHz</td>
<td class="org-left">3.2 GHz</td>
<td class="org-left">Cannon Lake (x64, 2018)</td>
<td class="org-left">LPDDR4 (3200 MT/s)</td>
<td class="org-left">GCC 9.1</td>
</tr>
</tbody>
</table>

我们的软件使用 C++17。我们在几个最新的编译器上测试（LLVM clang，GNU GCC, Microsoft Visual Studio）。我们的库经过很好的测试并包含在事实的系统比如 Yandex Clickhouse。我们在 Linux 系统上使用 GNU GCC 9.1 编译器，并开启 -O3 和 -march=native 选项。我们编译代码没有配置文件指导优化。所有的代码都是单线程的。我们禁止超线程

我们的实验假设 JSON 文档在内存中；我们忽略磁盘和网络访问。流行的磁盘（例如，NVMe）有超过 3GB/s 的带宽。在实际中，JSON 文档频繁进入网络。当前的网络标准允许超过 10GB/s 的速度且现代网络硬件可允许网络数据直接读取到内存总线。一个 JSON 扫描的高性能实现预想数据来自一个快速的磁盘或网络。当我们聚焦于速度，我们也期望更高效的解析器减少能量消耗

在看了一些解析器之后，我们选择 RapidJSON 和 sajson，两个开源 C++ 解析器。实际上，我们发现另一个 C++ 解析器，sajson 更快。它们都是成熟且高度优化的：分别创建于 2011 和 2012 年。sajson 解析器可被用来做静态或动态内存分配：静态版本更快，所以我们采用这种

统计我们自己的实现（simdjson），所有三个解析器都可解析 64 位浮点数。然而，sajson 只支持 32 位整型而 RapidJSON 和 simdJson 支持 64 位整型。RapidJSON 支持表示更大的 64 位浮点数的整型（以无损的方式）而我们的解析器（simdjson）和 sajson 不支持这样的整型

RapidJSON 可规范化字符串在新的缓存中或输入字节（insitu）中。我们发现在 insitu 模式上解析速度更快，所以我们呈现这些更好的数。相反的，sajson 只支持 insitu 解析。我们的实现不修改输入字节，没有 insitu 模式。所有三个解析器都做 UTF-8 的输入验证。然而，sajson 解析器做部分 UTF-8 验证，接受无效码点序列比如 0xb1 0x87

我们考虑另外的解析器但我们发现它们要么比 RapidJSON 慢要么不遵从 JSON 指导说明。例如，gason, jsmn, ultrajson 解析器接受 [0e+] 作为有效的 JSON。fastjson 和 ultrajson 解析器接受字符串中不转义的断行。其他解析器集成在更大的框架中，对它们进行公平地评测很困难。为简化，我们也不考虑 Java 或其他语言实现的解析器

RapidJSON 有编译时间选项来使用 SIMD 优化启动优化代码路径：这些优化跳过值或结构化字符间的空格。然而，我们发现这些编译器宏（RAPIDJSON<sub>SSE2</sub> 和 RAPIDJSON<sub>SSE42</sub>）在我们的测试中在系统上是对优化有害的。更进一步，它们在库中默认是被禁止的。这样我们不使用这些优化

除了 RapidJSON，我们发现没有库考虑使用 SIMD 指令。然而，我们期望在我们的测试中所有的库能在 SIMD 指令中受益，且编译器转换一些方便的代码到 SIMD 指令（例如，通过自动向量化）

我们不能直接比较 Mison 因为它们的软件是不公开的。然而，Mison 的作者报道在基频 3.5 GHz 的 Intel Xeon Broadwell-EP (E5-1620 v3) 上速度能上到 2 GB/s：例如，当部分解析 Twitter 数据时。Mison 没有尝试验证文档或整体解析它们但它也做了一些额外的处理来响应一个特殊的查询
...
解析速度依赖于 JSON 文档内容。对一个公平的估计，我们选择一个宽范围的文档

从 RapidJSON 作者那，我们获取了 3 个数据文件。我们有 canada.json 描述加拿大轮廓的地理 JSON 信息，它包含很多数值。我们有 citm_catalog.json 通常用来作为评测文件。最后，我们有 twitter.json 其内容为使用 Twitter API 的日本和中国的特征结果，它包含很多非 ASCII 字符。从 sajson 作者那，我们拿到一些更多的文件：apache_builds.json, github_events.json, instruments.json, mesh.json, mesh.pretty.json, update-center.json

我们也生成 number.json 作为一个随机浮点数值的大数组。创建 twitterespaced.json 其为 twitter.json 的缩减版去掉了所有非 ASCII 字符

这些文档需要进行很多数值解析或字符串规范处理。我们故意不考虑小文件（小于 50kB）。解析小文件不在我们的考虑范围
...
如下图，我们呈现每阶段对每个测试文件的周期数分布

-   标签1: 没有 utf8 在阶段 1 上的时间消耗，除了对 UTF-8 的验证。该时间跟其他文件处理相似，每输入字节在 0.5 到 1 周期之间。每输入字节的处理时间比有许多结构化和伪结构化字符的文档如 canada，marine_ik，mesh 和 random 要高。该不同可解释为基于索引提取的消耗比有许多结构化字符的文档要高
-   标签1: 只有 utf8 会有 UTF-8 验证处理。其他情况被忽略。在 random 文件中，UTF-8 验证是有大的成本的。该文件有一个相对高的非 ASCII 字符比例（20%）。相对的，twitter 文件只有 3%
-   标签2: core 只针对阶段 2 中的字符串和数值解析。其在文档 marine_ik 和 mesh 中更高，因其每输入字节中有很多结构化和非结构化字符
-   标签2: 字符串2: 数值对应时间消耗在阶段 2 的数值解析中。大约三分之一的 CPU 周期在文档 canada，marine_ik，mesh，mesh.pretty 和 numbers 消耗在数值解析中。其他文件中数值解析的消耗可忽略
-   标签2: 字符串相关字符串解析在阶段 2 中。字符串解析在文档 twitterespaced 中有相当大的成本。在这个文件中，所有非 ASCII 字符被转义使得字符串规范化更困难

![img](../img/cycles_per_input_dyte_process_test_files_in_simdjson.png)

大约一半的每输入字节 CPU 周期（0.5 到 3 周期）消耗在阶段 1。这个阶段多数依赖于向量处理（SIMD 指令）。这样至少一半的处理时间直接跟 SIMD 指令相关和无分支处理

解析器的运行时间依赖于 JSON 文档的特征。它对模型性能是有帮助的：例如，一个工程师可预测执行时间和预算。这样，我们基于我们的数据集文件使用线性回归方法。我们的数据集相对较小，但我们期望对一个简单的线性回归来说它足够大了

-   设 F 是浮点数值的数目
-   S 是结构化和半结构化元素
-   B 是字节数

对一个高精确度（ $ R^{2} \\ge 0.99 $），我们有如下常量模型：

-   阶段 1 运行时间（CPU 周期）在 Skylake 上是 $ 1.7 \\times S + 0.62 \\times B $，在 Cannon Lake 上是 $ 1.7 \\times S + 0.59 \\times B $
-   阶段 2 运行时间在 Skylake 上是 $ 19 \\times F + 8.7 \\times S + 0.31 \\times B $，在 Cannon Lake 上是 $ 17 \\times F + 9.1 \\times S + 0.29 \\times B $
-   总运行时间在 Skylake 上是 $ 19 \\times F + 11 \\times S + 0.92 \\times B $，在 Cannon Lake 上是 $ 17 \\times F + 11 \\times S + 0.88 \\times B $

输入字节的数量有一个小的系数（小于每输入字节 1 周期）但它对成本的贡献很大因为不是结构化元素或浮点数的字节更多

在我们的模型中，一个浮点数值不仅有一个成本，也产生一个伪结构化字符和几个字节的代价：这样每个数字有十几个周期的代价
...
SIMD 指令的主要好处是用更少的指令做更多的工作。这样我们期望我们的解析器使用更少的指令。下表中我们呈现了三个解析器解析各种文件需要的指令数。我们提供的数据来自于 Skylake 处理器，Cannon Lake 处理器也相似
...
<th scope="col" class="org-right">延迟</th>
<th scope="col" class="org-right">吞吐量</th>
</tr>
</thead>
...
<td class="org-left">加一个 8 位整数</td>
...
<tr>
<td class="org-left">vpalignr</td>
<td class="org-left">连接 16 字节块对</td>
<td class="org-right">1</td>
<td class="org-right">1</td>
</tr>
...
<tr>
<td class="org-left">vpcmpeqb</td>
<td class="org-left">8 位整数比较（=）</td>
<td class="org-right">1</td>
<td class="org-right">0.5</td>
</tr>
...
<tr>
<td class="org-left">vperm2i128</td>
<td class="org-left">洗牌整数</td>
...
<tr>
<td class="org-left">vpmaxub</td>
<td class="org-left">计算 8 位整数中最大值</td>
<td class="org-right">1</td>
<td class="org-right">0.5</td>
</tr>
...
<tr>
<td class="org-left">vpshufb</td>
<td class="org-left">洗牌字节</td>
<td class="org-right">1</td>
<td class="org-right">1</td>
</tr>
...
<tr>
<td class="org-left">vpsrld</td>
<td class="org-left">右移 32 位整数</td>
<td class="org-right">1</td>
<td class="org-right">1</td>
</tr>
...
<tr>
<td class="org-left">vpsrlw</td>
<td class="org-left">右移 16 位整数</td>
<td class="org-right">1</td>
<td class="org-right">1</td>
</tr>
...
<tr>
<td class="org-left">vpsubusb</td>
<td class="org-left">减去 8 位整数</td>
<td class="org-right">1</td>
<td class="org-right">0.5</td>
</tr>
...
<tr>
<td class="org-left">vptest</td>
<td class="org-left">测试为 0</td>
<td class="org-right">3</td>
<td class="org-right">1</td>
</tr>
</tbody>
...
一般来说，simdjson 使用大约一半数量 sajson 和 RapidJSON 的指令数。我们最快的竞争者 sajson 使用 1.5 到 3.5 倍的指令数。RapidJSON 使用 1.8 到 4.7 倍指令数。执行比 sajson 优势最小的文件是 marine_ik，mesh.pretty 和 twitterespaced。这些文件包含许多数值解析或成本高的字符串规范化

simdjson 解析器使用的指令数最少，但它使用更多的向量指令。上表提供了 simdjson 中 256 位向量指令的延迟和吞吐量。多数有最小的延迟（1 周期）且几个指令有高吞吐量：它们可每周期处理两次（vpaddb, vpand, vpcmpeqb 等）。所有这些向量指令至少每周期处理一次
...
下图列出了解析速度。在几个例子中，sajon 可超过 1 GB/s，RapidJSON 可超过 0.5 GB/s。在我们的 Skylake（3.4 GHz）处理器上，我们的解析器（simdjson）在 6 个例子中可达到甚至超过 2 GB/s，且对 gsoc2018，我们达到了 3 GB/s

![img](../img/speed_of_three_json_parsers.png)

解析的目的是访问文档中的数据。如果不能快速解析文档，则不能快速解析树
...
虽然使用 SIMD 指令解析的应用程序并不新颖，我们的结果显示流行的 JSON 解析器还没有完全利用其能力。我们期望我们的许多策略能让现有的 JSON 解析器比如 RapidJSON 获益。甚至可以集成我们解析器（simdjson）代码到现有的库中

JSON 是几个流行的数据格式比如 Protocol Buffers，XML，YAML，MessagePack，BSON，CSV 或 CBOR 之一。我们期望我们的许多思想可以应用在其他格式上

JSON 文档都是文本的。我们经常需要嵌入二进制内容到文档中。标准处理包含使用 base64 编码。Base64 数据可被 SIMD 指令快速解码。因为文本中的数值解析是高成本的，可以使用 base64 的二进制格式存储数值大数组

Intel 推出了新的指令集使用更宽的向量寄存器和更强大的指令（AVX-512）。我们的 Cannon Lake 处理器支持这些指令，包含 AVX512-VBMI 扩展，这与字节处理需要相关。进一步地研究了解访问 AVX-512 指令的优势

我们的许多策略可用到许多不同的处理器架构上。进一步地研究需要访问我们在其他处理器的性能，比如 ARM 和 POWER 系列

在我们当前的 simdjson 实现中，我们完全处理完阶段 1 之后才处理阶段 2。如果需要，阶段 1 可用线程平行处理，因为阶段 1 的块工作只需要局部输入信息
