...
哪个Cayley公式的证明最简单？当然取决于人们的喜好，然而，下面这个由Jim Pitman，加州伯克利大学的概率专家发现的，是一个很好的候选。它严谨地阐述了总是有一个机会发现一些在数学领域被认为很好理解的东西

该证明是一个双统计的复杂例子。我们将不统计树（或脊椎动物或相似物种），而是部分，或计划的有根树。什么是部分？形式上它是一个三元组$ (T,r,l) $，T是有顶点集合 $ V = \\{1,2,\\ldots,n \\} $（n是固定正整数）的一棵树，$ r \\in V $是根，lT标签为 $ 1, 2, \\ldots, n-1 $的边。即l是一个双射：$ l : E(T) \\to \\{1,2,\\ldots, n-1\\} $。这儿有一个部分的例子：

![img](../img/example_of_tree_part.png)

我们可想象我们在一个顶点集合V上开始一个空图形且我们通过添加边来生成一个有根树。标签l记为边添加的顺序

给定一棵树T，我们可用n种方式选择根，然后我们可选择标签l为 (n-1)! 种方式。因此部分的数量为 $ n(n-1)! \\kappa(K_ {n}) $

我们现在用另一种方式统计部分的数量。我们首先解释一个有根树为一个有向树，所有的边朝向根（这样的树有时被称为内分支）

![img](../img/example_of_oriented_tree.png)

我们记根为唯一的顶点不含任何箭头的箭尾；即唯一一个外度为0的顶点。树的任何朝向只有一个外度为0的顶点对应一颗有根树

现在我们考虑部分且我们想要统计部分的数量可从V的空图形通过添加n - 1个箭头获得

第一个箭头连接两个不同的顶点，且它可有n(n-1)种选择。第二个箭头不同于第一个，它得满足附加条件：因为所有箭头指向根，每个顶点外度最多为1，且因此第二个箭头的尾必须不同于第一个箭头的尾。第二个箭头的头可任意选择，因此我们有n(n-2)种可能

选择第k个箭头的限制是什么？我们必须服从如下两个规则：

（A）我们必须不造成循环（对应无向图，例如，忽略箭头方向）。这样，每个新选择的箭头必须连接图的两个不同的部分

（B）每个顶点这个进程的结束，对根的例外，必须为选择的箭头的一个的结尾。因为我们只有n-1个箭头，没有箭头有相同的尾

在当前图形的每一步每个部分只有一个外度为0的顶点。这是因为有m个顶点的一个部分只有m - 1个箭头，且每个顶点外度最多为1

如果我们根据(A)和(B)选择k个箭头，则当前图形有n - k个部分。因此对第k + 1个箭头我们有n(n - k - 1)个可能选择

在n - 1步之后我们获得一个部分，这样不同部分的数量为

$ \\prod^{n - 2}_ {k=0} n(n - k - 1) = (n - 1)!n^{n-1} $

这样我们得到Cayley公式：$ \\kappa(K_ {n}) = n^{n-2} $
