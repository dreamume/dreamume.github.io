...
$ \\begin{equation} V' = (V \\setminus \\{x, y\\}) \\cup \\{z\\} \\end{equation} $

$ \\begin{equation} E' = \\{ e \\in E: e \\cap \\{x, y\\} = \\emptyset \\} \\\\ \\cup \\{\\{z, t\\}: t \\in V \\setminus \\{x, y\\}, \\{t, x\\} \\in E \\text{ or } \\{t, y\\} \\in E \\} ; \\end{equation} $
...
$ \\begin{equation} c(s) = \\left\\{ \\begin{array}{ll} c'(s) & \\text{if } s \\notin {x, y, v} \\\\ c'(w) & \\text{if } x = x \\text{ or } x = y \\\\ i \\in \\{1, \\ldots, 5\\} \\setminus \\{ c'(w), c'(u), c'(t), c'(z) \\} & \\text{if } s = v \\end{array} \\right. \\end{equation} $
